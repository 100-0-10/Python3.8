# -*- coding: utf-8 -*-
"""
Created on Fri Apr 24 09:14:13 2020
...
### gensim ###
'''
text 데이터들의 topic modeling 하는 라이브러리.
즉 topic modelling에 대한 여러가지 기능이 구현되어 있다.

topic model (topic은 주제라는 뜻)
예를 들면
"트와이스는 너무 좋고 나는 모모를 좋아한다. " 라는 문장이 있으면
이 문장의 주제는 트와이스 혹은 모모 일 것이다.
그것은 관전에 따라 좀 다른데
일반적으로 사람은 이 문장의 주제가 무엇일까? 라고 질문하면
트와이스라고 이야기 할 관점이 크고
그 다음에 "트와이스 멤버의 모모" 라고 이야기 하는 사람도 있을 것이다.
이런식으로 텍스트 데이터 집단에서 해당 텍스트 집단들의 주제를 추출할 수 있거나
만들 수 있는 모델을 topic modelling 이라고 할 수 있다.
'''
...
for name, morphs_processor in morphs_processors :
     ...
     morphos = [morphs_processor.morphs(sentence) for sentence in sentences]
     elapsed_time = time.time() - start_time
     print('morphos_processor name = %20s, %.5f secs' % (name, elapsed_time))
...
예를 들어서, 다음 문장이 있으면,
"Family is not an important thidg. It's everything."

Family, important, thidg, everything 은 의미가 있다고 보고,
